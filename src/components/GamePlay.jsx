...
请根据玩家自定义的开局描述，结合世界书和角色设定，生成一个完整、详细的初始剧情（约2000字，按中文字符计算）。要求：
...
请生成一段引人入胜的初始剧情（约2000字，按中文字符计算），让玩家进入这个虚拟世界。要求：
1. 必须使用全中文输出
2. 必须使用第三人称视角进行长叙述
3. 详细描写场景、动作、心理、环境等
4. 严格遵循世界书设定和角色设定
5. 让玩家有代入感
6. 为后续互动留下空间
7. 严格控制字数在2000字左右，不要过长`
    }

    prompt += `\n\n重要：直接输出故事内容，不要有任何推理过程、思考过程、分析过程等元内容。不要输出"根据..."、"考虑到..."、"我认为..."等推理性语言。直接开始叙述。严格控制字数在2000字左右（按中文字符计算）。`
